...
class DataValidation:
    ...
    def validate_number_of_columns(self,dataframe:pd.DataFrame)->bool:
        try:
            ...
            if len(dataframe.columns)==len(self.schema_config["columns"]):
                return True
    ...
    def is_numerical_columns_exist(self, dataframe: pd.DataFrame) -> bool:
        try:
            logging.info("Checking if numerical columns exist in dataframe")
    ...
    def detect_dataset_drift(self,base_df,current_df,threshold=0.05)->bool:
        try:
            ...
            for column in base_df.columns:
                ...
                is_sample_dist=ks_2samp(d1,d2) ## to compare the sample distribution of the data
                ...
                report.update({column:{"p_value":is_sample_dist.pvalue,"drift_status":is_found}})
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            status=self.is_numerical_columns_exist(train_df)
            if not status:
                raise Exception("Numerical columns are not exist")
            status=self.is_numerical_columns_exist(test_df)
            if not status:
                raise Exception("Numerical columns are not exist")
